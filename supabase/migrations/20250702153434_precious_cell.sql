/*
  ...
  1. New Tables
    - `equipment` - Fitness equipment catalog with categories and images
    - `exercises` - Comprehensive exercise library with detailed information
    - `workout_templates` - Pre-built workout routines
    - `workout_template_exercises` - Junction table for template exercises
    - `workout_sessions` - User workout tracking
    - `exercise_sets` - Individual set tracking within workouts
    - `daily_goals` - Track daily fitness objectives
    - `nutrition_logs` - Food and calorie tracking
    - `body_measurements` - Physical progress tracking
    - `progress_photos` - Visual progress documentation
    - `user_achievements` - Gamification system
    - `user_favorites` - Bookmark system
    - `equipment_maintenance` - Equipment maintenance tracking

  2. Security
    - Enable RLS on all tables
    - Proper policies ensuring users can only access their own data
    - Public read access for equipment and exercises
    - Template sharing with public/private options
